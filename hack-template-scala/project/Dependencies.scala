...
object Dependencies {
  ...
  lazy val `redis-client` = Seq(
    "net.debasishg" %% "redisclient" % versions.redis
  )

  lazy val lift_json = Seq("net.liftweb" %% "lift-json" % "3.4.3")

  lazy val micrometer = Seq("io.micrometer" % "micrometer-core" % "1.7.1")
  ...
  lazy val all = Seq(
    ...
    Seq("io.kontainers" %% "micrometer-akka" % "0.12.2",
    "io.micrometer" % "micrometer-registry-prometheus" % versions.micrometerVersion,
